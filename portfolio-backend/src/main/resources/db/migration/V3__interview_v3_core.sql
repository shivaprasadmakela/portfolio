...
CREATE TABLE `portfolio`.`collection_types` (
    ...
    `NAME` VARCHAR(50) UNIQUE NOT NULL COMMENT 'Name of the collection type (e.g., CATEGORY, YOUTUBE_SET).',
    PRIMARY KEY (`ID`)
...
CREATE TABLE `portfolio`.`questions` (
    ...
    `IS_PUBLISHED` TINYINT(1) DEFAULT 0 COMMENT 'Publication status (0=Draft, 1=Published).',
...
-- SEED CORE DATA
-- =============================================================================
INSERT IGNORE INTO `collection_types` (`NAME`) VALUES ('CATEGORY'), ('YOUTUBE_SET');
