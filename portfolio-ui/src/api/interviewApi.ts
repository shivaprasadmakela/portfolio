...
export const interviewApi = {
    ...
    getQuestionBySlug: (slug: string): Promise<QuestionDto> =>
        apiClient<QuestionDto>(`${BASE_PATH}/questions/${slug}`),

    searchQuestions: (query: string): Promise<QuestionDto[]> =>
        apiClient<QuestionDto[]>(`${BASE_PATH}/questions/search?q=${encodeURIComponent(query)}`),
